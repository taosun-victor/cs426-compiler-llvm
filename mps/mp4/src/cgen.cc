...
//	};
	...
	while(actual->more(i)){
		...
		cout <<"actual " << i << "th type: " << actual->nth(i)->get_type()->get_string() << endl;
		operand *temp_param_op = env->lookup(actual->nth(i)->get_type());		
		if (temp_param_op != NULL) {
	...
	for(int i = 0; i < env->get_class()->function_names.size();i++){
		...
		if(std::string(env->get_class()->function_names.at(i)) == std::string(name->get_string())){
			...
			std::cerr << "CCCCCCCCCCCCC " << env->get_class()->vtable_types.at(i).get_name() << endl;
			if (env->get_class()->vtable_types.at(i).get_name()[0] == '%') {
				func_type_ptr = op_type(env->get_class()->vtable_types.at(i).get_name().substr(1),1);
				func_type_ptr.set_id(OBJ_PPTR);			
			}
			else {
				func_type_ptr = op_type();
				func_type_ptr.set_name(env->get_class()->vtable_types.at(i).get_name());
				func_type_ptr.set_id(OBJ_PPTR);
			}
	...
	/*
	op_type class_type; //= load_func_result.get_type();
	
	if (load_func_result.get_typename()[0] == '%') {
		class_type = op_type(env->get_class()->get_type_name() + "*", 1);
		class_type.set_id(OBJ_PPTR);	
	}
	else{
		class_type = load_func_result.get_type();
	}*/
	op_type class_type(env->get_class()->get_type_name() + "*");
